...
int think(int color){
	//will be used to tell whether or not poms should be discarded after sort
	...
	if (!green_count){ nonzerocount += 1;vector_add(&vect,1);} else {vector_add(&vect,0)}
	if (!orange_count){ nonzerocount += 1;vector_add(&vect,1);} else {vector_add(&vect,0)}
	if (!pink_count){ nonzerocount += 1;vector_add(&vect,1);} else {vector_add(&vect,0)}
	if (!blue_count){ nonzerocount += 1;vector_add(&vect,1);} else {vector_add(&vect,0)}
	
	switch (color){
		case 0:
			//green
			if (nonzerocount == 0){ return 1 } //if there arent any poms, no reason to discard
			else {
				if (nonzerocount >= 2 && !vector_get(&color, GREEN){
					if ()
				}
			}
